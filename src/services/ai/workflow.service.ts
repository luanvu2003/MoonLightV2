...
import { AIAnalyzerService, PersonAnalysisResult, GarmentAnalysisResult, WorkflowDecision } from './analyzer.service.js';
import { AIParsingService, HumanParsingResult } from './parsing.service.js';
import { AIMaskingService, ClothMaskResult } from './masking.service.js';
import { AIQualityService, QualityCheckResult } from './quality.service.js';
...
export class AIWorkflowService {
  ...
  static async runTryOnWorkflow(params: {
    ...
    while (attempt <= MAX_RETRIES) {
      ...
      // Gọi VTON Model
      const modelOut = await vtonModelCaller(
        personImage,
        garmentImage,
        workflow,
        product?.name || 'luxury outfit',
        clothMask,
        attempt
      );
      ...
      const tQC = Date.now();
      qualityResult = AIQualityService.evaluateQuality(
        Boolean(finalResultUrl),
        finalProvider,
        personAnalysis,
        garmentAnalysis,
        clothMask,
