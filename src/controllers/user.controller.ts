...
export class UserController {
   ...
   */
  static async getCustomersWithStats(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const userList = users.map((user) => {
        ...
        const userOrders = orders.filter((o: any) => {
          ...
          if (orderCustId && orderCustId === uId) return true;
          if (uPhone && orderPhone && uPhone === orderPhone) return true;
          if (uEmail && orderEmail && uEmail === orderEmail) return true;
          return false;
        });
      ...
      const registeredPhones = new Set(
        users.map((u) => (u.phone ? u.phone.replace(/[\s.-]/g, '').trim() : '')).filter(Boolean)
      );
      ...
      orders.forEach((o: any) => {
        ...
        if (oCustId && users.some((u) => u._id.toString() === oCustId)) return;
        if (oPhone && registeredPhones.has(oPhone)) return;
   ...
   */
  static async getUserOrderHistory(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (id.startsWith('guest_')) {
        ...
        const orConditions: any[] = [{ customerId: targetUser._id }];
        if (targetUser.phone) {
