...
function sanitizeProductData(p) {
    if (!p)
        return p;
...
export class ProductController {
    static async getAll(req, res, next) {
        try {
                ...
                if (mongoose.connection.readyState === 1) {
                    ...
                    if (totalInDb > 0) {
                        if (page && limit) {
                            ...
                            sendPaginated(res, products.map(sanitizeProductData), {
                                page: pageNum,
                                limit: limitNum,
                                total,
            ...
            if (size) {
                const sizeList = Array.isArray(size) ? size : String(size).split(',').map(s => s.trim());
                fallbackList = fallbackList.filter((p) => p.variants?.some((v) => v.sizes?.some((s) => sizeList.includes(s.size) && s.stock > 0)));
            }
            if (search) {
    ...
    static async update(req, res, next) {
        try {
            ...
            if (!updatedProduct && req.body.name) {
                updatedProduct = await Product.findOneAndUpdate({ name: req.body.name }, req.body, { new: true, runValidators: true });
            }
            if (!updatedProduct) {
